...
CWidget::CWidget(QWidget *parent) : QWidget(parent)
{
    ...
    multiChoiceButton->setFont(font);
...
void CWidget::on_listView_doubleClicked(const QModelIndex &index)
{
        ...
        extension == ".png" || extension == ".pcx" || extension == ".BMP")
    {
        ...
        ldtParams[RESOLUTION]->setText(QString::number(img.physicalDpiX())); //разрешение
    }
}
...
void CWidget::onMultiChoiceCliked()
{
    ...
    for(int i = 0; i < list.size(); i++)
    {
        ...
        twInfo->setItem(i, 5, new QTableWidgetIntItem(QString::number(im.physicalDpiX())));
    }
